...
class ProfileDetailView(LoginRequiredMixin,DetailView):
    ...
    def get_context_data(self,**kwargs):
        ...
        rel_s=Relationship.objects.filter(reciver=profile)
...
class ProfileListView(LoginRequiredMixin,ListView):
    ...
    def get_context_data(self,**kwargs):
        ...
        context["rel_receiver"]=rel_reciver #rel_receiver Bu şekilde olması gerekiyor. yazarken e harfini yazmayı unutmuşum.
        context["rel_sender"] = rel_sender
...
def send_invatation(request):
    if request.method=="POST":
        ...
        rel=Relationship.objects.create(sender=sender, reciver=receiver,status="send")
...
def remove_from_friends(request):
    if request.method=='POST':
        ...
        rel = Relationship.objects.get(
            (Q(sender=sender) & Q(reciver=receiver)) | (Q(sender=receiver) & Q(reciver=sender))
        )
        rel.delete()
        return redirect(request.META.get('HTTP_REFERER'))
    return redirect('profiles:my-profile-view')
