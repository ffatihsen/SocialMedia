...
class ProfileManager(models.Manager):
    ...
    def get_all_to_invate(self,sender):
        profiles=Profile.objects.all().exclude(user=sender) #istek gönderen kişi hariç Tüm profilleri al
        profile=Profile.objects.get(user=sender) #istek gönderen kişiyi profile değişkenine ata
        qs=Relationship.objects.filter(Q(sender=profile) | Q(reciver=profile))
        print("qs:",qs)
        print("********************")
...
class RelationshipManager(models.Manager):
    def invatations_recived(self,reciver):
        qs=Relationship.objects.filter(reciver=reciver,status="send")
        return qs
